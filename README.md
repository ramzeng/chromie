...
在工作区菜单中打开“工作区设置”，切换到“MCP”页面即可启用。MCP 默认关闭；首次启用后仍为只读模式，开启写入权限后才能编辑数据或联网同步。页面会生成当前安装环境可直接使用的 MCP 客户端配置。
...
写操作由主进程顺序执行。历史快照始终只读；MCP 不提供删除能力，也不接受、读取或返回 API Key、Secret Key、Passphrase 等同步凭据。
